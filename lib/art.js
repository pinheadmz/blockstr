...
class Art {
  ...
  async drawMessage() {
    const imgPath = path.join(__dirname, '..', 'html', 'static', 'message.png');
    const stream = fs.createReadStream(imgPath);

    const img = await PImage.decodePNGFromStream(stream);
    const imgData = img.getContext('2d').getImageData(0, 0, img.width, img.height);
    const destData = this.ctx.getImageData(0, 0, this.img.width, this.img.height); // full canvas

    const src = imgData.data;
    const dst = destData.data;

    const offsetX = 70;
    const canvasWidth = this.img.width;

    for (let y = 0; y < img.height; y++) {
        for (let x = 0; x < img.width; x++) {
            const srcIdx = (y * img.width + x) * 4;
            const dstX = x + offsetX;
            if (dstX >= canvasWidth) continue; // skip out-of-bounds
            const dstIdx = (y * canvasWidth + dstX) * 4;

            const alpha = src[srcIdx + 3] / 255;
            dst[dstIdx + 0] = src[srcIdx + 0] * alpha + dst[dstIdx + 0] * (1 - alpha);
            dst[dstIdx + 1] = src[srcIdx + 1] * alpha + dst[dstIdx + 1] * (1 - alpha);
            dst[dstIdx + 2] = src[srcIdx + 2] * alpha + dst[dstIdx + 2] * (1 - alpha);
            dst[dstIdx + 3] = 255; // or keep existing if you want to preserve output alpha
        }
    }
